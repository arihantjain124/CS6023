...
__device__ volatile unsigned node_counter_prev = 0;
__device__ volatile unsigned block_inc = 0;
__device__ volatile unsigned last_node_per_level = 0;
__device__ volatile unsigned last_thread_blocks;
ofstream outfile; // The handle for printing the output

/******************************Write your kerenels here ************************************/


__global__ void nodes_per_level(int *csrList,int *offset,int *apr,int *aid,int V,int L,int *activeVertex){
    ...
    for(int j=0;j<L;j++){
        ...
        if( ((apr[level_id] == 0 && level == 0) || (level > 0  && level_id>=node_counter && level_id<=last_node_per_level) ) && level_id<V ){
            ...
            if(aid[level_id]>=apr[level_id]){
                if( level_id == node_counter ||  level_id == last_node_per_level || aid[level_id-1]>=apr[level_id-1] || aid[level_id+1]>=apr[level_id+1] ){
...
nodes_per_level<<<gridDimx,threadDimx>>>(d_csrList,d_offset,d_apr,d_aid,V,L,d_activeVertex);
