...
__global__ void per_row_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    long int id_a = (int)(index / m);
    long int id_b = index % m;
    if(id_a < m && id_b < m)
    {
        for(int i = 0;i<n;i++){
            for(int j = 0;j<n;j++){
...
__global__ void per_column_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    long int index = (blockIdx.x * blockDim.x * blockDim.y)  +  (threadIdx.y * blockDim.x)  +   threadIdx.x;
    long int id_a = (int)(index / n);
    long int id_b = index % n;
    if(id_a < n && id_b < n)
    {
        for(int i = 0;i<m;i++){
            for(int j = 0;j<m;j++){
...
__global__ void per_element_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    long int id_c; 
    long int r,c;
    id_c = (blockDim.x * gridDim.x * blockDim.y * blockIdx.y) + (blockIdx.x * blockDim.x * blockDim.y)  +  (threadIdx.y * blockDim.x)  +   threadIdx.x;
    if(id_c < (m*n*m*n))
    {
        r =  id_c / (m*n);
        c =  id_c % (m*n);
